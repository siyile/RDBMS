...
int RBFTest_private_6(PagedFileManager &pfm) {
    ...
    std::cout << "after reopen: R W A - " << readPageCount1 << " " << writePageCount1 << " " << appendPageCount1
              << std::endl;
    assert((readPageCount1 - readPageCount <= 2) && "Read counter should be correct.");
    assert((writePageCount1 - writePageCount <= 2) && "Write counter should be correct.");
    assert((appendPageCount1 - appendPageCount == 0) && "Append counter should be correct.");
